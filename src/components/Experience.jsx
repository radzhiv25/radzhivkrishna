...
const ExperienceCard = ({ exp, index }) => {
  ...
  return (
    <div ref={cardRef} className="relative flex items-start mb-8">
      ...
      >
        {/* Title */}
        <h3 className={`font-semibold mb-2 ${index === 0 ? "text-lg text-black" : "text-base text-gray-800"}`}>
          {exp.title}
        </h3>

        {/* Card */}
        <div className="border rounded-lg p-4 shadow-sm bg-white hover:shadow-md hover:scale-[1.01] transition-all duration-300">
          ...
          {exp.techStack && (
            <div className="mt-3 text-sm">
              ...
              <span className="text-gray-600 ml-1">{exp.techStack.join(", ")}</span>
            </div>
          )}
        </div>
      </motion.div>
