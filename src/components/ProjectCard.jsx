  ...
  return (
    <div className={`h-max p-2 border rounded-md break-inside-avoid ${className} transition-all mb-5`}>
      ...
      <span className="flex items-center justify-between">
        <Link to={link}>
          <h3 className="text-xl font-semibold hover:text-gray-500">{name}</h3>
        </Link>
        {category !== "UI/UX" && (
          <Link to={source || "#"} className="hover:text-gray-500">
            <LuGithub className='size-5' />
          </Link>
        )}
      </span>
      <p className="mt-1 mb-3 text-base leading-tight">{description}</p>
      <div className="flex items-end gap-2 text-xs overflow-x-auto scrollbar-hide">
        ...
        <span className="flex items-center gap-2">
          {skills.map((skill, index) => (
            <p key={index} className='border border-gray-300 rounded py-0.5 px-1 text-xs'>{skill}</p>
          ))}
        </span>
      </div>
    </div>
